...
*An example hydrograph of the improvement in discharge predictions when we incorporate SWOT data. The blue and red lines compare experiments where we trained a model with and without SWOT data, and then tested this model on new **unseen** basins, testing the spatial generalization. This hydrograph shows the the Swan River near Big Fork, Montana- a glacially fed river with some impact from hydroelectic damming. With the SWOT data, the model shifts peak discharge later in the season which more closely matches the gauge data. It's likely that the ERA5 data do not represent the right conditions for accurately predicting meltwater for this small basin.*